...
| `/submit_guide` | 攻略ガイド記事を投稿 | 複数の入力方法に対応（下記参照） |
...
コマンド実行後、以下の3つの入力方法から選択できます：

| 入力方法 | 説明 | 画像対応 |
|---|---|---|
| 📄 **ファイル添付** | `.md` ファイルをチャンネルに送信 | ❌ |
| ✏️ **テキスト入力** | Discordモーダルで直接Markdown入力 | ❌ |
| 🔗 **URL入力** | note.com / Google Docs の URL を貼付け | ✅ note.comのみ |

**※ URL入力（note.com）の場合:**
- 記事内の画像は自動でダウンロードされます
- 同じマッチアップのガイドを再投稿すると、古い画像は自動削除されます
